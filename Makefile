...
KBUILD_CFLAGS   := -Wall -Wundef -Werror=strict-prototypes -Wno-trigraphs \
		   ...
		   -Wno-format-security \
		   -std=gnu89
KBUILD_CPPFLAGS := -D__$(PROJECT)__ -D__KERNEL__
...
export KBUILD_$(PROJECT)_HEAD := $(head-y)
export KBUILD_$(PROJECT)_INIT := $(init-y)
export KBUILD_$(PROJECT)_MAIN := $(core-y) $(libs-y2) $(drivers-y) $(net-y) $(virt-y)
export KBUILD_$(PROJECT)_LIBS := $(libs-y1)
export KBUILD_LDS          := arch/$(SRCARCH)/kernel/$(project).lds
export LDFLAGS_$(project)
export PLATFORM_LIBS
# used by scripts/package/Makefile
export KBUILD_ALLDIRS := $(sort $(filter-out arch/%,$($(project)-alldirs)) arch Documentation include samples scripts tools test_bench)

$(project)-deps := $(KBUILD_LDS) $(KBUILD_$(PROJECT)_HEAD) $(KBUILD_$(PROJECT)_INIT) $(KBUILD_$(PROJECT)_MAIN) $(KBUILD_$(PROJECT)_LIBS) $(PLATFORM_LIBS)
	
# Final link of vmlinux with optional arch pass after final link
cmd_link-$(project) =                                                 \
	$(CONFIG_SHELL) $< $(LD) $(KBUILD_LDFLAGS) $(LDFLAGS_$(project))

$(project): scripts/link-$(project).sh $($(project)-deps) FORCE
...
archprepare: archheaders archscripts prepare1 scripts test_bench
...
define filechk_timestamp.h
	(if test -n "$${SOURCE_DATE_EPOCH}"; then \
		...
		if test -n "$${DATE}"; then \
			LC_ALL=C $${DATE} -u -d "$${SOURCE_DATE}" +'#define $(PROJECT)_DATE "%b %d %C%y"'; \
			LC_ALL=C $${DATE} -u -d "$${SOURCE_DATE}" +'#define $(PROJECT)_TIME "%T"'; \
			LC_ALL=C $${DATE} -u -d "$${SOURCE_DATE}" +'#define $(PROJECT)_TZ "%z"'; \
			LC_ALL=C $${DATE} -u -d "$${SOURCE_DATE}" +'#define $(PROJECT)_DMI_DATE "%m/%d/%Y"'; \
			LC_ALL=C $${DATE} -u -d "$${SOURCE_DATE}" +'#define $(PROJECT)_BUILD_DATE 0x%Y%m%d'; \
		else \
			return 42; \
		fi; \
	else \
		LC_ALL=C date +'#define $(PROJECT)_DATE "%b %d %C%y"'; \
		LC_ALL=C date +'#define $(PROJECT)_TIME "%T"'; \
		LC_ALL=C date +'#define $(PROJECT)_TZ "%z"'; \
		LC_ALL=C date +'#define $(PROJECT)_DMI_DATE "%m/%d/%Y"'; \
		LC_ALL=C date +'#define $(PROJECT)_BUILD_DATE 0x%Y%m%d'; \
	fi)
endef
...
# Test Bench options
PHONY += test_bench
ifeq ($(CONFIG_TEST_BENCH),y)
test_bench: prepare1 FORCE
	$(Q)$(MAKE) $(build)=test_bench $@
else
test_bench: FORCE
	@:
endif
...
clean-dirs      := $(addprefix _clean_, . $($(project)-alldirs) test_bench)

PHONY += $(clean-dirs) clean archclean $(project)clean
$(clean-dirs):
...
help:
	...
	@echo  '* $(project)	  - Build the bare kernel'
	...
	@echo  '  $(project)release	  - Output the release version string (use with make -s)'
	@echo  '  $(project)version	  - Output the version stored in Makefile (use with make -s)'
	...
	@$(MAKE) -f $(srctree)/test_bench/Makefile help
	@echo  ''
