...
int32_t parse_usb_config(const uint8_t*           confBuf,
		...
		zusb_EndpointDescriptor** epIsocIn,
			...
			if((itfDesc.bInterfaceNumber == interfaceNum) &&
				...
				if((epDesc.bEndpointAddress & ZUSB_ENDPOINT_DIR_MASK) == ZUSB_DIR_IN) {
					if((epDesc.bmAttributes & ZUSB_ENDPOINT_XFERTYPE_MASK) == ZUSB_ENDPOINT_XFER_BULK) {
						...
						*epIsocIn = new zusb_EndpointDescriptor;
						memcpy(*epIsocIn, &epDesc, ZUSB_DT_ENDPOINT_SIZE);
						printf("  ISOC IN\n");
					} else if ((epDesc.bmAttributes & ZUSB_ENDPOINT_XFERTYPE_MASK) == ZUSB_ENDPOINT_XFER_INT) {
						*epIntIn = new zusb_EndpointDescriptor;
						memcpy(*epIntIn, &epDesc, ZUSB_DT_ENDPOINT_SIZE);
						printf("  INT IN\n");	
					} else {
						printf("not support ep: %d\n", epDesc.bmAttributes & ZUSB_ENDPOINT_XFERTYPE_MASK);
					}
				...
				printf("Don't initialize endpoints of other interface alternate settingn");
			}
		}
